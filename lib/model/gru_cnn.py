...
from lib.util.preprocessing import extract_encoder_model


class GRU_CNN:
    def __init__(self, word_embedding_map, word_tokenizer, model_config):
    ...
    def create_model(self):
        # Declaration for GRU-based code encoder
        code_encoder_input = Input(shape=(self.model_config.max_code_len,) , dtype='int32')
        code_embedding_layer = extract_encoder_model(load_model(os.path.join('data', 'embeddings', 'code',
                                                                             'docstring_pretrained_py.hdf5')))
        embedded_code_sequences = code_embedding_layer(code_encoder_input)
        c_dense1 = Dense(300, activation='relu')(embedded_code_sequences)
        code_encoder = Model(code_encoder_input, c_dense1)

        # Freeze encoder if required
        if self.model_config.freeze_encoder:
            for l in code_encoder.layers:
                l.trainable = False
    ...
    def train(self, train_xw1, train_xw2, train_xc1, train_xc2, train_y, evaluate_xw1, evaluate_xw2, evaluate_xc1,
        ...
        checkpoint_callback = ModelCheckpoint(filepath="data/checkpoints/gru_cnn/%s_%d.hdf5" %
        ...
        self.model.load_weights(filepath="data/checkpoints/gru_cnn/%s_%d.hdf5" % (self.model_config.dataset, iteration))
